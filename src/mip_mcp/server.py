...
class MIPMCPServer:
    ...
    async def _initialize_pyodide(self):
        """Check basic requirements (simplified)."""
        if self._pyodide_initialized:
            return

        # Simple check: can we run Node.js?
        import shutil

        if not shutil.which("node"):
            logger.warning(
                "Node.js not found. Please install Node.js to use optimization features."
            )
            return

        logger.info("✓ Node.js available for Pyodide execution")
    ...
    def run(self, show_banner: bool = True):
        ...
        # Pyodide initialization is now handled on-demand by each executor
