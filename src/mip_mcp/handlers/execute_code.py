...
def suppress_stdout_for_mcp():
    ...
    is_mcp_mode = (
        os.environ.get("MCP_MODE") == "1"
        or "--mcp" in sys.argv
        or "mcp-server" in " ".join(sys.argv)
    )
    ...
    async def mcp_progress_callback(progress: SolverProgress):
        ...
        try:
            ...
            if "elapsed" in (progress.message or ""):
                ...
                time_factor = min(
                    progress.time_elapsed / 30.0, 0.5
                )  # Up to 50% extra over 30s
    ...
    try:
        ...
        if include_solver_output:
            ...
            with suppress_stdout_for_mcp():
                solution = await solver_instance.solve_from_file(
            ...
            if hasattr(solution, "solver_info") and solution.solver_info:
                solver_output_captured = solution.solver_info.get(
                    "detailed_output", "No detailed output available"
                )
            else:
                solver_output_captured = "No solver output available"

            logger.info(
                f"Generated solver output summary, length: {len(solver_output_captured)}"
            )
        else:
            # Suppress output for clean MCP protocol
            with suppress_stdout_for_mcp():
                solution = await solver_instance.solve_from_file(
                    file_path, capture_output=False
                )
        ...
        if validate_solution and solution.is_optimal:
            try:
                ...
                logger.info(
                    "Solution validation not yet implemented for Pyodide executor"
                )
                problem_obj = None
                ...
                if problem_obj:
                    ...
                    validation_result = validator.validate_solution(
                        problem_obj, solution.model_dump()
                    )

                    # Create validation model and add to solution
                    solution.validation = SolutionValidation(**validation_result)
                    logger.info(
                        f"Solution validation completed: valid={validation_result['is_valid']}"
                    )
                else:
                    logger.info(
                        "Solution validation skipped - not implemented for Pyodide yet"
                    )
    ...
    try:
        ...
        while not solving_task.done():
            try:
                ...
                progress_response = await asyncio.wait_for(
...
async def get_solver_info_handler(
    ...
    try:
        ...
        for solver_name in available_solvers:
            try:
                solver_instance = SolverFactory.create_solver(
                ...
                solvers_info[solver_name] = SolverInfo(
                    name=solver_name, available=False, error=str(e)
                )

        return SolverInfoResponse(
            status="success", solvers=solvers_info, default_solver=default_solver
        )
...
async def validate_mip_code_handler(
    code: str, config: dict[str, Any] | None = None
) -> ValidationResponse:
    ...
    try:
        ...
        return ValidationResponse(
            status="error", message=f"Validation failed: {e}", issues=[], is_valid=False
        )
