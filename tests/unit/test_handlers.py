...
from tests.fixtures.sample_problems import INVALID_SYNTAX, SIMPLE_LP
...
class TestExecuteCodeHandler:
    ...
    def handler_config(self):
        ...
        return {
            "execution": {"timeout": 30, "memory_limit": "512MB"},
            "solvers": {
                "default": "scip",
                "timeout": 3600,
    ...
    async def test_execute_mip_code_handler_success(self, handler_config):
        ...
        mock_solver.get_solver_info = Mock(
            return_value={"name": "SCIP", "version": "8.0.3", "available": True}
        )
    ...
    async def test_execute_mip_code_handler_execution_error(self, handler_config):
        ...
        with patch(
            ...
            result = await execute_mip_code_handler(
                code=INVALID_SYNTAX, config=handler_config
            )
    ...
    async def test_execute_mip_code_handler_solver_error(self, handler_config):
        ...
        mock_solver = AsyncMock()
        mock_solver.solve_from_file = AsyncMock(
            return_value=Mock(
                status="error",
        ...
        def mock_executor_with_progress():
            ...
            async def execute_with_progress(*args, **kwargs):
                ...
                if callback:
                    ...
                    callback(
                        SolverProgress(
                            stage="modeling", percentage=30.0, elapsed_time=1.0
                        )
                    )
                    callback(
                        SolverProgress(
                            stage="solving", percentage=80.0, elapsed_time=3.0
                        )
                    )
                    callback(
                        SolverProgress(
                            stage="finished", percentage=100.0, elapsed_time=5.0
                        )
                    )
