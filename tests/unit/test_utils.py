...
class TestConfigManager:
    ...
    def test_config_manager_env_override(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            default_config = config_dir / "default.yaml"
            default_data = {
                "server": {"name": "default-name", "version": "1.0.0"}
            }
            with open(default_config, 'w') as f:
            ...
            env_data = {
                "server": {"name": "env-override", "port": 9000}
            }
...
class TestSolutionValidator:
    ...
    def test_validate_solution_with_error(self):
        ...
        assert isinstance(validation, dict)
        assert validation["is_valid"] is True  # Error handling returns True with error info
